    ...
    try:
        if BOTLOG:
            Config.ZEDUBLOGO = await zedub.tgbot.send_file(
                BOTLOG_CHATID,
                "https://telegra.ph/file/69bd274c55068807c758e.jpg",
                caption="**•⎆┊تـم بـدء تشغـيل سـورس لــيدر الخاص بك .. بنجاح 🧸♥️**",
                buttons=[(Button.url("𝗟𝗘𝗮𝗗𝗲𝗥", "https://t.me/V_P_N_8"),)],
            )
    except Exception as e:
        LOGS.error(e)
        return None
...
async def mybot():
    ...
    if bot_name.endswith("Assistant"):
        ...
        try:
            ...
            await asyncio.sleep(1)
            await bot.send_message("@BotFather", "V_P_N_8")
            ...
            await bot.send_message("@BotFather", botname)
            await asyncio.sleep(1)
            await bot.send_message("@BotFather", f"- بـوت لــيدر المسـاعـد ♥️🦾 الخـاص بـ  {bot.me.first_name} ")
            await asyncio.sleep(3)
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        ...
        _, groupid = await create_supergroup(
            "كـروب السجـل لــيدر", zedub, Config.TG_BOT_USERNAME, descript, photozed
        )
